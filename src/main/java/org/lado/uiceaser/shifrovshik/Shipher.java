package org.lado.uiceaser.shifrovshik;

public class Shipher {
    public static final char[] RusAlphabet = {'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ё', 'Ж',
            'З', 'И', 'Й', 'К', 'Л', 'М', 'Н', 'О', 'П', 'Р', 'С', 'Т',
            'У', 'Ф', 'Х', 'Ц', 'Ч', 'Ш', 'Щ', 'Ъ', 'Ы', 'Ь', 'Э', 'Ю', 'Я', '.', ',', ':', '!', '?', ' '};

    public static String chipher(String text, int key) {
    ...
    public static String deChipher(String text, int key) {
        char[] str = text.toCharArray();
        char[] result = new char[str.length];
    ...
    public static String deChipher(String text, String checkString) {
        int maxSize = 9;
        for (int i = 1; i < maxSize; i++) {
            String decrypted = Shipher.deChipher(text, i);
